...
function StorylineSectionCluesContent({ clues }) {
  return (<>
    <div className="StoryLineContent">
      ...
      <div style={{ display: "flex", flexDirection: "row", alignItems: "flex-start", width: "35%" }}>Durée : {clues.displayAfter} sec.</div>
    </div>
  </>)
}

function StorylineSectionInputContent({ input }) {
  console.log(input.answersId, "position")
  return (<>
    <div className="StoryLineContent">
      <div style={{ display: "flex", flexDirection: "row", alignItems: "flex-start", width: "80px" }}>{input.answersId}</div>
    ...
    case "Écran input":
      return (<>
        <div className="StoryLineLayoutBox">
          ...
          <> {input?.inputs.map((input, index2) => <StorylineSectionInputContent key={input.id} input={input} index={index} index2={index2}/>)}</>
